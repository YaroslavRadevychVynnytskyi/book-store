...
```PUT:    /api/cart/cart-items/{cartItemId}``` - updated quantity of books.\
```DELETE: /api/cart/cart-items/{cartItemId}``` - deletes book from cart.
5. **Order management endpoints:**
    * Administrator available:\
```PATCH:  /api/orders/{id}``` - updated status of the order.
    * User available:\
...
This chapter provides general instructions for setting up the ***Online Book Store API***.
...
___
# API work display
In this section you will be shown a couple of basic use cases of the application
...
it required me to set up a new development environment and transfer all my files to the new computer. 
